  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (idEditando) {
        ...
        mostrarMensaje('Materia actualizada', 'exito');
      } else {
        await api.post('/materias', data);
        mostrarMensaje('Materia creada', 'exito');
      }
      cancelarEdicion();
      fetchDatos();
    } catch (error) {
    ...
    <div className="space-y-6 max-w-7xl mx-auto h-full flex flex-col">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          <div className="bg-blue-950 p-3 rounded-xl shadow-md"><BookMarked className="text-white w-6 h-6" /></div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8 flex-1 min-h-0">
          ...
          <div className={`transition-all duration-300 h-full ${mostrarFormulario ? 'xl:col-span-3' : 'xl:col-span-4'}`}>
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden h-full flex flex-col">
              {materias.length === 0 ? (
                <div className="flex-1 p-12 flex items-center justify-center text-center text-gray-500 font-medium">Aún no tienes materias. Haz clic en "Nueva Materia".</div>
              ) : (
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {materias.map((materia) => (
                        <tr key={materia.id_materia} className="hover:bg-gray-50/50">
                          <td className="p-4 font-bold text-gray-900">{materia.nombre}</td>
                          <td className="p-4 text-sm text-gray-600">{materia.profesor || '-'}</td>
                          <td className="p-4 text-sm text-blue-900 font-medium"><span className="bg-blue-50/50 px-2 py-1 rounded-md border border-blue-100">{materia.periodo}</span></td>
                          <td className="p-4 flex justify-center gap-2">
                            <button onClick={() => editarMateria(materia)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 className="w-5 h-5" /></button>
                            <button onClick={() => eliminarMateria(materia.id_materia)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="w-5 h-5" /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {mostrarFormulario && (
            <div className="xl:col-span-1 overflow-y-auto pr-2 animate-in slide-in-from-right-8 duration-300">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold text-gray-900 mb-6 border-b pb-4">{idEditando ? 'Editar Materia' : 'Nueva Materia'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Nombre</label>
                    <input type="text" value={nombre} onChange={(e) => setNombre(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none" required />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Profesor (Opcional)</label>
                    <input type="text" value={profesor} onChange={(e) => setProfesor(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Periodo Escolar</label>
                    <select value={idPeriodo} onChange={(e) => setIdPeriodo(e.target.value)} disabled={idEditando !== null} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none disabled:opacity-50">
