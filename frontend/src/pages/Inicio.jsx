...
export default function Inicio() {
  ...
  const fetchDatos = async () => {
    try {
      const [resTareas, resHorarios, resMaterias] = await Promise.all([api.get('/tareas'), api.get('/horarios'), api.get('/materias')]);
      setTareas(resTareas.data); setHorarios(resHorarios.data); setMaterias(resMaterias.data);
    } catch (error) {
  ...
  const clasesHoy = horarios.filter(h => h.dia_semana === diaHoyAbrev).sort((a, b) => a.hora_inicio.localeCompare(b.hora_inicio));

  const tareasOrdenadas = [...tareas].sort((a, b) => {
    if (a.completada === b.completada) return new Date(a.fecha_entrega) - new Date(b.fecha_entrega);
  ...
  if (loading) return <div className="p-8 text-center text-gray-500 dark:text-gray-400 font-medium animate-pulse">Cargando tu información...</div>;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white dark:bg-gray-900 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-800 transition-colors">
        <div>
          <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight mb-2">
            ¡Hola, <span className="text-blue-950 dark:text-blue-400">{user?.nombre?.split(' ')[0] || 'Estudiante'}</span>!
          </h1>
                  ...
                  <div key={idx} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm relative group transition-colors">
                      <div className="flex items-center justify-between mb-1">
                        <span className="font-bold text-blue-950 dark:text-blue-300">{clase.materia}</span>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-lg w-full shadow-2xl animate-in fade-in zoom-in-95 duration-200 border dark:border-gray-800" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 text-gray-800 dark:text-gray-200 font-medium"><CalendarIcon className="w-5 h-5 text-gray-400 dark:text-gray-500" />{new Date(tareaSeleccionada.fecha_entrega).toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</div>
              </div>
              <div>
