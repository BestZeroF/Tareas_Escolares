...
export default function Tareas() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      mostrarMensaje(error.response?.data?.error || 'Error', 'error');
    }
  };
  ...
  const eliminarTarea = async (id) => {
    ...
    try { await api.delete(`/tareas/${id}`); fetchDatos(); } 
    ...
    <div className="space-y-6 max-w-7xl mx-auto h-full flex flex-col">
      ...
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-3">
          <div className="bg-blue-950 p-3 rounded-xl shadow-md"><CheckSquare className="text-white w-6 h-6" /></div>
      ...
      {materias.length === 0 ? (
        <div className="bg-orange-50 border border-orange-200 p-8 rounded-3xl text-center">
          ...
          <p className="text-orange-700">Debes tener materias registradas para crear tareas.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8 flex-1 min-h-0">
          ...
          <div className={`transition-all duration-300 h-full ${mostrarFormulario ? 'xl:col-span-3' : 'xl:col-span-4'}`}>
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden h-full flex flex-col">
              {tareas.length === 0 ? (
                <div className="flex-1 p-12 flex items-center justify-center text-center text-gray-500 font-medium">Excelente, no tienes tareas. Haz clic en "Nueva Tarea" para registrar una.</div>
              ) : (
  ...
  >
    {t.completada ? <CheckCircle className="w-7 h-7 mx-auto" /> : <Circle className="w-7 h-7 mx-auto" />}
                          ...
                          <td className="p-4"><span className="text-sm text-blue-900 font-medium bg-blue-50/50 px-2 py-1 rounded-md border border-blue-100">{t.materia}</span></td>
                          <td className="p-4"><div className="flex items-center gap-2 text-sm text-gray-600"><Clock className="w-4 h-4 text-gray-400" />{formatearFecha(t.fecha_entrega)}</div></td>
                          <td className="p-4 flex justify-center gap-1">
                            {!t.completada && <button onClick={() => editarTarea(t)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg"><Edit2 className="w-5 h-5" /></button>}
                            <button onClick={() => eliminarTarea(t.id_tarea)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg"><Trash2 className="w-5 h-5" /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          {mostrarFormulario && (
            <div className="xl:col-span-1 overflow-y-auto pr-2 animate-in slide-in-from-right-8 duration-300">
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                <h2 className="text-xl font-bold text-gray-900 mb-6 border-b pb-4">{idEditando ? 'Editar Tarea' : 'Nueva Tarea'}</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Título</label>
                    <input type="text" value={titulo} onChange={(e) => setTitulo(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none" required />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Descripción</label>
                    <textarea value={descripcion} onChange={(e) => setDescripcion(e.target.value)} rows="3" className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none resize-none"></textarea>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Entrega</label>
                    <input type="date" value={fechaEntrega} onChange={(e) => setFechaEntrega(e.target.value)} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none" required />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1">Materia</label>
                    <select value={idMateria} onChange={(e) => setIdMateria(e.target.value)} disabled={idEditando !== null} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-950 outline-none disabled:opacity-50">
